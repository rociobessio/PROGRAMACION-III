<?php 
    ...
    include_once "./31/db/accesoDatos.php";
...
//********************************************** FUNCIONES *************************************************************
        ...
        /**
         * Me permitira buscar y retornar un usuario mediante su ID
         * 
        ...
        public static function buscarUsuario($idUsuario) {
            ...
            $consulta = $objAccessoDatos->RetornarConsulta("SELECT * FROM usuarios WHERE id = :id");
        ...
        public static function verificarUsuario($mail, $clave) {
            ...
            if ($consulta->rowCount() > 0) {
                echo "Verificado";  //-->Quire decir que existe
                return true;
            } elseif (!$consulta->rowCount() && self::existeUsuarioPorMail($mail)) {
                echo "Error en los datos";//-->Falla algo en los datos ingresados
                return false;
            } else {
                echo "Usuario no registrado"; //-->Directamente no esta registrado
                return false;
            }
        }
        ...
        /**
         * Me permitira modificar un usuario en la db.
         * 
         * @return bool true si pudo, false sino.
         */
        public static function modificarUsuario($mail,$claveNueva,$claveVieja){
            $objetoAccesoDato = AccesoDatos::obtenerObjetoAcceso();
    
            //-->Valido que exista el email y la claveVieja
            $consultaVerificacion = $objetoAccesoDato->retornarConsulta("SELECT id FROM usuarios WHERE email = :mail AND clave = :claveVieja");
            $consultaVerificacion->bindValue(':mail', $mail, PDO::PARAM_STR);
            $consultaVerificacion->bindValue(':claveVieja', $claveVieja, PDO::PARAM_STR);
            $consultaVerificacion->execute();
            
            if ($consultaVerificacion->rowCount() > 0) {
                //-->Si existe cambio la claveVieja por la claveNueva
                $consultaActualizacion = $objetoAccesoDato->retornarConsulta("UPDATE usuarios SET clave = :claveNueva WHERE email = :mail");
                $consultaActualizacion->bindValue(':claveNueva', $claveNueva, PDO::PARAM_STR);
                $consultaActualizacion->bindValue(':mail', $mail, PDO::PARAM_STR);
                
                if ($consultaActualizacion->execute()) {
                    return true;//-->La clave se pudo modificar la clave
                } else {
                    return false;//-->No pudo
                }
            } else {
                return false;//-->El mail y la claveVieja no existe en la db
            }
        }
